...
$id = $mid_err  = $customer_err  = $litres_err  = $plitre_err  = $date_err  = $total_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_date)){
     ...
     if(empty($input_total)){
         $total_err = "Please enter total milk price.";     
     } else{
         $total = $input_total;
     }
    ...
    && empty($total_err)){
        ...
        $sql = "INSERT INTO dkmilk (mid, customer, litres, plitre, date, total)
...
<head>
    <meta charset="UTF-8">
    <title>Create Cow Sales Record</title>
